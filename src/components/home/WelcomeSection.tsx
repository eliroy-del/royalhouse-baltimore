...
export function WelcomeSection() {
  return (
    <Section tone="cream" spacing="md" id="welcome">
      <Container>
        <div className="grid items-center gap-6 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1fr)] lg:gap-8">
          <Reveal y={24} className="relative">
            <div className="relative mx-auto aspect-[3/4] w-full max-w-sm overflow-hidden mask-arch bg-navy-900 lg:max-w-none">
              <Image
            ...
            <span
              aria-hidden="true"
              className="absolute -left-3 -top-3 -z-10 hidden size-32 rounded-full bg-gold-100 lg:block"
            />
          </Reveal>
          ...
          <div>
            ...
            <Reveal delay={0.06}>
              <h2 className="mt-3 font-display text-[clamp(1.75rem,3.6vw,2.75rem)] font-light leading-[1.03] tracking-tight text-navy-900">
                You belong here{" "}
            ...
            <Reveal delay={0.1}>
              <p className="mt-3 max-w-xl text-[0.9375rem] leading-relaxed text-navy-900/70">
            ...
            <Stagger className="mt-5 grid gap-x-6 gap-y-2.5 sm:grid-cols-2">
              {welcomes.map((item) => (
                <StaggerItem key={item} className="flex items-start gap-2.5">
                  <span className="mt-0.5 flex size-4 shrink-0 items-center justify-center rounded-full bg-gold-500/15 text-gold-700">
                  ...
                  <span className="text-[0.875rem] leading-snug text-navy-900/75">{item}</span>
                </StaggerItem>
              ))}
            </Stagger>
